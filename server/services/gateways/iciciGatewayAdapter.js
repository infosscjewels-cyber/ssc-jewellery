const buildMissingHandlerError = (handlerName = '') => {
    const error = new Error(`ICICI gateway handler "${String(handlerName || '').trim()}" is not configured.`);
    error.code = 'ICICI_GATEWAY_HANDLER_MISSING';
...
const createIciciGatewayAdapter = (handlers = {}) => ({
